...
while ($res=mysqli_fetch_assoc($query)) {
	...
	if ($sch_sch_id == $cur_sch_id){
		...
		if ($res['sch_priority']==1){
			break;
		}
		
		echo 'Clicked schedule: '.$sch_sch_id;
		echo 'Previous schedule:'.$previous_sch_id;
...
//		echo $sql;
		...
		$sql="update sig_schedules set sch_priority=sch_priority+1 
		WHERE sch_user_id = '$user_user_id' and sch_group_id = '$sch_group_id'  and sch_sch_id = '$previous_sch_id'  ";
//		echo $sql;
		mysqli_query($con,$sql);
		break;		
	}

	$previous_sch_id=$res['sch_sch_id'];
}
